...
/*
  ...
  local i = 1
  repeat
    local b = (s & 0x80000000)
    s = (s << 1) & 0xFFFFFFFF
    if b ~= 0 then s = s ~ 0x04C11DB7 end   -- bit reversed 0x04C11DB7
    for j = 0, i - 1 do
...
{
  return (s << 8) ^ crc32_table[x ^ ((s >> 24) & 0xff)];
}
...
int main()
{
  printf("%08x\n", crc32_bulk((const uint8_t []){ 0x00 }, 1));  // 4e08bfb4
  printf("%08x\n", crc32_bulk((const uint8_t []){ 0x01 }, 1));  // 4ac9a203
  printf("%08x\n", crc32_bulk((const uint8_t []){ 0x00, 0x00 }, 2));  // 00b7647d
  return 0;
}
#endif
